...
export default function DashboardPage() {
  ...
  if (loading) return <Box minH="100vh" display="flex" justifyContent="center" alignItems="center" bgGradient="linear(to-r, #74ebd5, #acb6e5)"><Spinner size="xl" color="blue.500" /></Box>;
  if (error) return <Box minH="100vh" display="flex" justifyContent="center" alignItems="center" bgGradient="linear(to-r, #74ebd5, #acb6e5)"><Text color="red.500">{error}</Text></Box>;
  ...
  return (
    <Box minH="100vh" p={8} bgGradient="linear(to-r, #74ebd5, #acb6e5)">
      ...
      <Text mb={4} fontSize="md" color="gray.700">Logged in as: <b>{role}</b></Text>

      {(role === "Admin" || role === "Editor") && (
        <Box mb={8} p={6} border="2px dashed #3498db" borderRadius="2xl" textAlign="center" bg="whiteAlpha.800" _hover={{ bg: "whiteAlpha.900" }}>
          <Text mb={3} fontWeight="semibold" color="gray.700">Drag & Drop files here or click to upload</Text>
          <input type="file" onChange={handleUpload} style={{ opacity: 0, position: "absolute", width: "100%", height: "100%", cursor: "pointer" }} />
      ...
      <Box mb={4} display="flex" justifyContent="flex-end" alignItems="center">
        ...
        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} style={{ padding: "6px 10px", borderRadius: "8px", border: "1px solid #ccc", cursor: "pointer" }}>
          <option value="created_at">Upload Date</option>
          <option value="name">Name</option>
        </select>
      </Box>

      {currentAssets.length === 0 ? <Text color="gray.600">No assets found.</Text> :
        <Grid templateColumns="repeat(auto-fill, minmax(250px, 1fr))" gap={6}>
          {currentAssets.map((asset) => (
            <GridItem key={asset.id} bg="white" borderRadius="2xl" boxShadow="md" p={4} cursor="pointer" transition="0.3s" _hover={{ boxShadow: "xl", transform: "translateY(-4px)" }} onClick={() => openModal(asset)}>
              {asset.preview_url ? <Image src={asset.preview_url} alt={asset.name} borderRadius="xl" mb={3} /> :
                <Box bg="gray.100" h="150px" borderRadius="xl" display="flex" alignItems="center" justifyContent="center"><Text color="gray.500">No Preview</Text></Box>}
              ...
              <Box mt={2}>{asset.tags?.map((tag, i) => <Tag key={i} size="sm" colorScheme="blue" mr={1}>{tag}</Tag>)}</Box>
            </GridItem>
          ))}
        </Grid>
      }
      ...
      <Box mt={6} display="flex" justifyContent="center" alignItems="center">
        <Button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} mr={2}>Prev</Button>
        <Text mx={2} fontSize="sm">Page {currentPage} of {totalPages}</Text>
        <Button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} ml={2}>Next</Button>
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} size="4xl" isCentered>
        ...
        <ModalContent borderRadius="2xl" p={4}>
          ...
          <ModalBody display="flex" flexDir="column" alignItems="center">
            ...
            <Box mt={3}>{selectedAsset?.tags?.map((tag, i) => <Tag key={i} size="sm" colorScheme="blue" mr={1}>{tag}</Tag>)}</Box>
          </ModalBody>
        </ModalContent>
      </Modal>
    </Box>
